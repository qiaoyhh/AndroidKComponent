...
import com.tencent.wesing.image_compress.compressor.CompressorFactory
...
class ImgCompressTask extends DefaultTask {
    ...
    @Internal
    com.tencent.wesing.image_compress.util.Logger log
    @Internal
    ...
    @TaskAction
    def run() {
        log = com.tencent.wesing.image_compress.util.Logger.getInstance(project.getProject())
        ...
        def wayList = config.way.split(',')
        for (String way : wayList) {
            ResultInfo resultInfo = new ResultInfo()
            CompressorFactory.getCompressor(way, project).compress(project, unCompressFileList, config, resultInfo)
            log.i("$way compress finish, compressed:${resultInfo.compressedSize} files  skip:${resultInfo.skipCount} Files  before total size: ${com.tencent.wesing.image_compress.util.FileUtils.formetFileSize(resultInfo.beforeSize)}" +
                    " after total size: ${com.tencent.wesing.image_compress.util.FileUtils.formetFileSize(resultInfo.afterSize)} save size: ${com.tencent.wesing.image_compress.util.FileUtils.formetFileSize(resultInfo.beforeSize - resultInfo.afterSize)}  ")
        }
...
//                log.i("fileName ${fileName}")
                ...
                def newMd5 = com.tencent.wesing.image_compress.util.FileUtils.generateMD5(it)
                //过滤已压缩文件
